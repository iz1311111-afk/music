...
export async function POST(request) {
  ...
  const { title, cols, rows, items, user_id, author } = body || {};
  ...
  const res = await fetch(`${SUPA_URL}/rest/v1/grids`, {
    ...
    body: JSON.stringify({ id, title: String(title || '').slice(0, 60), cols: Number(cols) || 6, rows: Number(rows) || 4, items: clean, user_id: user_id ? String(user_id).slice(0, 40) : null, author: author ? String(author).slice(0, 20) : null })
  });
